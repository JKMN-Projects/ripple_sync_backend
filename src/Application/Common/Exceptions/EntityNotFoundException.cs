...
public class EntityNotFoundException : Exception
{
        ...
        where TEntity : class
    {
        ...
        return keyName is null
            ? new EntityNotFoundException(entityType, key)
            : new EntityNotFoundException(entityType, key, keyName);
    }
    ...
    public static EntityNotFoundException ForEntity<TEntity, TProp>(object key, Expression<Func<TEntity, TProp>> keyName)
        where TEntity : class
    {
        return keyName.Body is not MemberExpression memberExpression
            ? throw new ArgumentException("Expression must be a member expression.", nameof(keyName))
            : ForEntity<TEntity>(key, memberExpression.Member.Name);
    }
}
